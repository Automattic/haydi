...
- **Model policy.** `Haydi_Settings_Page` registers the `haydi_editor_model` option and an administrator-only **Settings → Haydi** page through the native Settings API. `Haydi_Model_Policy` resolves that stored provider/model pair against current connector choices and localizes the resulting browser policy; when set, `assets/admin.js` selects and disables the configured model in the chat picker. It is deliberately a UI policy, not server-side model authorization.
