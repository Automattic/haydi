...
- **Access control.** Browser pages, AJAX handlers, and logged-in REST requests all use `haydi_current_user_can_access()`. Its default `edit_others_posts` capability admits standard Editors and Administrators; sites can replace it through `haydi_access_capability`. Bearer-token REST/MCP authorization remains independent of WordPress roles because possession of a valid token is the authorization boundary.

- **Editor model policy.** `Haydi_Settings_Page` registers the `haydi_editor_model` option and an administrator-only **Settings → Haydi** page through the native Settings API. `Haydi_Model_Policy` resolves that stored provider/model pair against current connector choices and localizes the resulting browser policy. For a non-administrator, `assets/admin.js` selects and disables the configured model; Administrators retain the normal picker. This is deliberately a UI policy, not server-side model authorization.
...
- **Remote Access architecture.**
  ...
  - *REST API:* `Haydi_Rest_Api` is instantiated at the bottom of `haydi.php` alongside `Haydi_Ajax_Handlers`; all routes share one `check_permission()` callback (Bearer token OR the configured Haydi access capability for a logged-in WordPress user). The file-action, query, and PHP modules add their routes via `rest_api_init`.
