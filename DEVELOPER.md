...
- **Access control.** Browser pages and AJAX handlers use `haydi_current_user_can_access()`, a hard-coded `manage_options` check that admits only Administrators. It is deliberately not filterable: `run_php` can grant the calling user any capability, including `manage_options` itself, so a lower floor would not be a meaningful boundary and offering one as a hook only invites insecure configurations. REST/MCP requests require a Bearer token unconditionally (`haydi_is_authorized_api_request()`); an authenticated WordPress session is never sufficient on its own, regardless of capability.

- **Model policy.** `Haydi_Settings_Page` registers the `haydi_editor_model` option (name kept for back-compat) and an administrator-only **Settings → Haydi** page through the native Settings API. `Haydi_Model_Policy` resolves that stored provider/model pair against current connector choices and localizes the resulting browser policy; when a model is pinned, `assets/admin.js` selects and disables it in the chat picker for every user, administrators included — the setting guards against costly or unsuitable model choices, and an administrator who wants a different model changes the setting itself. It is deliberately a UI policy, not server-side model authorization.
