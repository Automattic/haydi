...
- **Access control.** Browser pages and AJAX handlers use `haydi_current_user_can_access()`. Its default `manage_options` capability admits only Administrators; sites can widen it through the `haydi_access_capability` filter, but the floor is deliberately Administrator because `run_php` can grant the calling user any capability, including `manage_options` itself — a lower default would not be a meaningful boundary. REST/MCP requests require a Bearer token unconditionally (`haydi_is_authorized_api_request()`); an authenticated WordPress session is never sufficient on its own, regardless of capability.

- **Editor model policy.** `Haydi_Settings_Page` registers the `haydi_editor_model` option and an administrator-only **Settings → Haydi** page through the native Settings API. `Haydi_Model_Policy` resolves that stored provider/model pair against current connector choices and localizes the resulting browser policy. For a non-administrator, `assets/admin.js` selects and disables the configured model; Administrators retain the normal picker. This only takes effect on sites that have widened `haydi_access_capability` to admit non-administrators — it is deliberately a UI policy, not server-side model authorization.
